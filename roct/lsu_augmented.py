...
class LSUAugmented(LSU, object):
    ...
    def __init__(
        ...
        super(LSUAugmented, self).__init__(
            formula, solver=solver, expect_interrupt=expect_interrupt, verbose=verbose
        )
    ...
    def solve(self):
        ...
        while self.oracle.solve_limited(expect_interrupt=self.expect_interrupt):
            ...
            if self.verbose:
                print(
                    "o {0} {1}".format(self.cost, time.time() - start_time), flush=True
                )
